...
let get_categories = () => {
  let category = []
  Object.keys(info.items).forEach((key, index) => {
    let object_content = info.items[key]
    if(Object.keys(object_content).length == 2) {
      object_content.code = key
      category.push(
        object_content
      )
    }
...
$(".cat_selector").click(function() {
  let category = $(this).attr("code")
  remove_active_category()

  $(this)
    .css('color', 'rgb(51, 100, 156)')
  ...
  $.each(current_items, (index, value) => {
    ...
    $("article").append(`
      <section>
        <div class="content">
          <h3>${ value.title }</h3>
          <h4>${ value.manufacturer }</h4>
          <div class="footer-content">PHP ${ value.price }</div>
          <div class="footer-content text-right">Released: ${ release_date.getDate() }/${ release_date.getMonth() }/${ release_date.getFullYear() }</div>
        </div>
      </section>
    `)
  })
